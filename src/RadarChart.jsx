...
import { ATTRS, pointsNeeded } from './gameLogic'

export default function RadarChart({ levelPoints, level }) {
  const size = 220
  const cx = size / 2
  const cy = size / 2
  const r = 80
  const needed = pointsNeeded(level)
  ...
  const dataPoints = ATTRS.map((a, i) => {
    const ratio = Math.min(levelPoints[a.key] / needed, 1)
    return getPoint(i, ratio, r)
  })

  const labelPoints = ATTRS.map((a, i) => getPoint(i, 1.35, r))
